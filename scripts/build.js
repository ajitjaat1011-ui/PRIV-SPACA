...
try {
  ...
  html = html.replace(/style\.css(\?v=[^"']*)?/g, 'style.min.css?v=94');
  html = html.replace(/app\.js(\?v=[^"']*)?/g, 'app.min.js?v=94');
  // Write modified HTML
...
try {
  ...
  sw = sw.replace(/style\.css(\?v=[^']*)?/g, 'style.min.css?v=94');
  sw = sw.replace(/app\.js(\?v=[^']*)?/g, 'app.min.js?v=94');
