...
try {
  ...
  html = html.replace(/style\.css(\?v=[^"']*)?/g, 'style.min.css?v=85');
  html = html.replace(/app\.js(\?v=[^"']*)?/g, 'app.min.js?v=85');
  // Write modified HTML
...
try {
  ...
  sw = sw.replace(/style\.css(\?v=[^']*)?/g, 'style.min.css?v=85');
  sw = sw.replace(/app\.js(\?v=[^']*)?/g, 'app.min.js?v=85');
  sw = sw.replace(/priv-spaca-v\d+/g, 'priv-spaca-v85');
