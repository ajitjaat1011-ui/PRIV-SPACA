...
try {
  ...
  html = html.replace(/style\.css(\?v=[^"']*)?/g, 'style.min.css?v=81');
  html = html.replace(/app\.js(\?v=[^"']*)?/g, 'app.min.js?v=81');
  // Write modified HTML
...
try {
  ...
  sw = sw.replace(/style\.css(\?v=[^']*)?/g, 'style.min.css?v=81');
  sw = sw.replace(/app\.js(\?v=[^']*)?/g, 'app.min.js?v=81');
  sw = sw.replace(/priv-spaca-v\d+/g, 'priv-spaca-v81');
