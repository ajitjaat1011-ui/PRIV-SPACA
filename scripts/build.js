...
try {
  ...
  html = html.replace(/style\.css(\?v=[^"']*)?/g, 'style.min.css?v=79');
  html = html.replace(/app\.js(\?v=[^"']*)?/g, 'app.min.js?v=79');
  // Write modified HTML
...
try {
  ...
  sw = sw.replace(/style\.css(\?v=[^']*)?/g, 'style.min.css?v=79');
  sw = sw.replace(/app\.js(\?v=[^']*)?/g, 'app.min.js?v=79');
  sw = sw.replace(/priv-spaca-v\d+-[a-z]+/g, 'priv-spaca-v79');
