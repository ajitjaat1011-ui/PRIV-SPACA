...
try {
  ...
  html = html.replace(/style\.css(\?v=[^"']*)?/g, 'style.min.css?v=87');
  html = html.replace(/app\.js(\?v=[^"']*)?/g, 'app.min.js?v=87');
  // Write modified HTML
...
try {
  ...
  sw = sw.replace(/style\.css(\?v=[^']*)?/g, 'style.min.css?v=87');
  sw = sw.replace(/app\.js(\?v=[^']*)?/g, 'app.min.js?v=87');
  sw = sw.replace(/priv-spaca-v\d+/g, 'priv-spaca-v87');
