...
try {
  ...
  html = html.replace(/style\.css(\?v=[^"']*)?/g, 'style.min.css?v=96');
  html = html.replace(/app\.js(\?v=[^"']*)?/g, 'app.min.js?v=96');
  // Write modified HTML
...
try {
  ...
  sw = sw.replace(/style\.css(\?v=[^']*)?/g, 'style.min.css?v=96');
  sw = sw.replace(/app\.js(\?v=[^']*)?/g, 'app.min.js?v=96');
