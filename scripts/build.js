...
try {
  ...
  html = html.replace(/style\.css(\?v=[^"']*)?/g, 'style.min.css?v=130');
  html = html.replace(/app\.js(\?v=[^"']*)?/g, 'app.min.js?v=130');
  // Write modified HTML
...
try {
  ...
  sw = sw.replace(/style\.css(\?v=[^']*)?/g, 'style.min.css?v=130');
  sw = sw.replace(/app\.js(\?v=[^']*)?/g, 'app.min.js?v=130');
  sw = sw.replace(/priv-spaca-v[\d.]+/g, 'priv-spaca-v104');
...
try {
  ...
  appjs = appjs.replace(/const APP_VERSION = 'priv-spaca-v[\d.]+'/g, "const APP_VERSION = 'priv-spaca-v104'");
