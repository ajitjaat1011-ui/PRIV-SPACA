...
- Production persistence is Supabase-only. Missing `SUPABASE_DB_URL`/`SUPABASE_URL`, `FIELD_KEY` or a strong `JWT_SECRET` must make protected API routes fail closed.
...
Database backup retention is an external platform control and must be enabled/monitored in the Supabase project. Do not write plaintext JSON backups into this public repository.

At least monthly, an operator should:

1. Create a timestamped Supabase database backup using the approved private project.
...
- Rotate Supabase, Cloudflare, Cloudinary, VAPID and administrative credentials immediately on suspected exposure and on the organization’s regular schedule.
...
3. For suspected database-only compromise, rotate the Supabase credentials, preserve forensic access logs and assess encrypted-envelope exposure separately from Worker-secret exposure.
