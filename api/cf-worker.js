...
// ---------- Reels (short vertical videos, with optional Instagram URL embed) ----------
// Two flavors of reel are supported:
//   (A) Self-uploaded: video data URL is uploaded via /api/upload-photo (Cloudinary → GitHub),
//       then stored in ps_posts with kind: 'reel'. Fully self-hosted, plays in-app.
//   (B) Instagram URL: we proxy the IG official oEmbed endpoint to get the embed HTML +
//       thumbnail + author. We store the embed HTML as a string and the thumbnail.
//       When the user opens the reel we show a "tap to open in Instagram" card — Instagram
//       does not allow direct in-app playback of their videos (no oEmbed video URL is
//       exposed, and scraping their JS player breaks ToS). This is the only legal option.
// Both kinds live in ps_posts with `kind: 'reel'` so they share the same list/feed.
app.get('/api/reels', requireAuth, async (c) => {
  try {
    const sdb = isTursoConfigured() ? await fetchTursoMirror() : await fetchDatabase();
    const sourceUsers = sdb.users || [];
    const sourcePosts = sdb.posts || [];
    ...
    sourceUsers.forEach(u => { if (u.id !== myId && Array.isArray(u.blocked) && u.blocked.includes(myId)) blockedMe.add(u.id); });
    const list = sourcePosts
      .filter(p => !p.deletedAt && p && p.kind === 'reel' && !myBlocked.has(p.userId) && !blockedMe.has(p.userId))
      .slice().sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))
      .map(p => {
        const author = sourceUsers.find(u => u.id === p.userId);
        const a = author ? sanitizeUser(author) : (p.authorSnapshot || { id: p.userId, displayName: 'Member', username: 'member' });
        return {
          id: p.id,
          userId: p.userId,
          kind: 'reel',
          source: p.source || 'self',                   // 'self' | 'instagram'
          videoUrl: p.videoUrl || null,                 // for self-uploaded reels
          igUrl: p.igUrl || null,                       // for IG reels
          igEmbed: p.igEmbed || null,                   // for IG reels (HTML from oEmbed)
          igThumbnail: p.igThumbnail || null,            // for IG reels (poster image)
          igAuthor: p.igAuthor || null,                 // for IG reels (display name)
          igAuthorUrl: p.igAuthorUrl || null,           // for IG reels (link to profile)
          text: p.text || '',
          music: p.music || null,
    ...
    return c.json({ reels: list });
  } catch (e) {
    console.error('[reels list] failed:', e && e.message);
    return c.json({ reels: [] }, 200); // never break the UI on read failure
  }
});

// Save a reel. Two modes:
//   { videoUrl, text?, music? }         → self-uploaded reel
//   { igUrl, igEmbed, igThumbnail, ... } → Instagram reel
// The video URL must already be a persisted CDN URL (from /api/upload-photo).
app.post('/api/reels/add', requireAuth, async (c) => {
  try {
    const body = await c.req.json().catch(() => ({}));
    const { videoUrl, text, music, igUrl, igEmbed, igThumbnail, igAuthor, igAuthorUrl } = body;
    const myId = c.get('userId');
    const db = await fetchDatabase();
    const author = db.users.find(u => u.id === myId);
    const snap = author ? { id: author.id, username: author.username, displayName: author.displayName, photoUrl: author.photoUrl || '' } : null;
    ...
    let post;
    if (igUrl) {
      // Instagram reel — must have a valid ig embed + thumbnail (we proxy oEmbed server-side)
      const safeIgUrl = (() => { try { const u = new URL(String(igUrl)); if (!/instagram\.com$/i.test(u.hostname)) return null; return u.toString(); } catch (_) { return null; } })();
      if (!safeIgUrl) return c.json({ error: 'Invalid Instagram URL' }, 400);
      if (typeof igEmbed !== 'string' || igEmbed.length < 10) return c.json({ error: 'Missing Instagram embed data — try again' }, 400);
      if (typeof igThumbnail !== 'string' || !isSafeImageUrl(igThumbnail, { allowData: false })) return c.json({ error: 'Missing Instagram thumbnail' }, 400);
      // Limit embed size to keep the DB row small (Instagram embed HTML is typically ~3KB)
      const cleanEmbed = String(igEmbed).slice(0, 8192);
      post = {
        id: uid('reel'),
        userId: myId,
        kind: 'reel',
        ...
        igUrl: safeIgUrl,
        igEmbed: cleanEmbed,
        igThumbnail: String(igThumbnail).slice(0, 1024),
        igAuthor: typeof igAuthor === 'string' ? sanitizeText(igAuthor, 80) : null,
        igAuthorUrl: typeof igAuthorUrl === 'string' ? String(igAuthorUrl).slice(0, 512) : null,
      ...
      // Self-uploaded reel — must have a persisted video URL
      ...
      if (!cleanVideo) return c.json({ error: 'A video file is required' }, 400);
      const cleanMusic = music && typeof music === 'object' && music.title ? {
        title: sanitizeText(music.title, 60),
        artist: sanitizeText(music.artist || '', 60),
        audio: isSafeMediaUrl(music.audio, { allowData: false }) ? String(music.audio).trim().slice(0, 1024) : '',
    ...
    if (!post) return c.json({ error: 'Empty reel' }, 400);

    db.posts.push(post);
    const enriched = { ...post, likeCount: 0, commentCount: 0, author: snap || { id: myId, displayName: 'Member', username: 'member' } };
    _broadcastEvent('new_post', { post: enriched }, myId);
    const persisted = await saveDatabaseVerified(db, d => (d.posts || []).some(p => p.id === post.id), 4, { skipSecondarySync: true });
    if (isPersist() && !persisted) return c.json({ error: 'Reel storage unavailable. Please retry.' }, 503);
    if (isTursoConfigured()) {
      await tursoUpsertPosts([post]);
    }
    return c.json({ reel: enriched });
  } catch (e) {
    console.error('[reels add] failed:', e && e.message);
    return c.json({ error: 'Could not save reel' }, 500);
  }
});

// Proxy Instagram's public oEmbed endpoint. We hit it server-side because the
// frontend can't reach api.instagram.com directly (CORS). The endpoint is rate-
// limited by Instagram — when it returns 429 we surface a friendly error.
app.get('/api/instagram/oembed', requireAuth, async (c) => {
  const url = c.req.query('url');
  if (!url) return c.json({ error: 'Missing url' }, 400);
  let safeUrl;
  try {
    const u = new URL(String(url));
    if (!/instagram\.com$/i.test(u.hostname)) return c.json({ error: 'URL must be on instagram.com' }, 400);
    // Only allow reel/p/tv URLs (anything else, IG oEmbed returns 404)
    if (!/\/reel\/|\/p\/|\/tv\//i.test(u.pathname)) return c.json({ error: 'Only Instagram reel/post URLs are supported' }, 400);
    safeUrl = u.toString();
  } catch (_) {
    return c.json({ error: 'Invalid URL' }, 400);
  }
  const oembedUrl = 'https://api.instagram.com/oembed/?url=' + encodeURIComponent(safeUrl);
  try {
    const r = await fetch(oembedUrl, {
      headers: { 'User-Agent': 'PRIV-SPACA/1.0' },
      cf: { cacheTtl: 3600, cacheEverything: true }, // Cloudflare cache for 1h (IG rate-limits us hard)
    });
    if (r.status === 429) return c.json({ error: 'Instagram is rate-limiting our server. Try again in a minute.' }, 503);
    if (r.status === 404) return c.json({ error: 'Instagram could not find that post — make sure it\'s public.' }, 404);
    if (!r.ok) {
      const t = await r.text().catch(() => '');
      console.warn('[ig oembed]', r.status, t.slice(0, 200));
      return c.json({ error: 'Instagram oEmbed failed (' + r.status + ')' }, 502);
    }
    ...
    console.error('[ig oembed] fetch failed:', e && e.message);
    return c.json({ error: 'Could not reach Instagram' }, 502);
  }
});
